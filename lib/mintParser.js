...
// process(tranjson) once for each transaction found in the stream where tranjson is
...
function parse(stream, process, args, done) {
  var alldone = new AllDone(done);
  stream.pipe(es.split())
    .pipe(es.map(function(line) {
      var tranjson = buildTran(line);
      if (tranjson) {
        alldone.i++;
        process(tranjson, args.importId, alldone.cb.bind(alldone));
      }
    }));
}
